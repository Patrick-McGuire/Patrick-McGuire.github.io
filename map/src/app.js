...
function renderPointFeature(point, index, selectedKey) {
  ...
  return '<details class="tree-item point' + (selected ? ' selected' : '') + '" data-tree-key="' + key + '"' + (selected ? ' open' : '') + '>' +
    ...
    '<span class="tree-title">' + escapeHtml(point.name) + '</span>' +
    ...
    '</summary>' +
    '<div class="tree-body">' +
    prop('Name', '<input type="text" data-kind="point" data-field="name" data-index="' + index + '" value="' + escapeHtml(point.name) + '">') +
...
function renderPathFeature(path, index, selectedKey) {
  ...
  return '<details class="tree-item ' + (isSegment ? 'segment' : 'path') + (selected ? ' selected' : '') + '" data-tree-key="' + key + '"' + (selected ? ' open' : '') + '>' +
    ...
    '<span class="tree-title">' + escapeHtml(path.name) + '</span>' +
    ...
    '</summary>' +
    '<div class="tree-body">' +
    prop('Name', '<input type="text" data-kind="path" data-field="name" data-index="' + index + '" value="' + escapeHtml(path.name) + '">') +
...
function handleElementTreeChange(event) {
  ...
  if (kind === 'point' && Number.isInteger(index)) {
    ...
    if (field === 'name') selectFeature({ kind: 'point', point: state.pointRecords[index] }, false);
  }
  if (kind === 'path' && Number.isInteger(index)) {
    updatePathField(index, field, control.value);
    if (field === 'name') selectFeature({ kind: 'path', path: state.drawnPaths[index] }, false);
  }
  if (kind === 'path-point') {
...
function renderFileList() {
  ...
  ui.fileList.innerHTML = state.files.map((file, index) => {
    ...
    return '<div class="file-item' + (current ? ' current' : '') + '">' +
      '<div><div class="file-name">' + escapeHtml(file.name || 'Untitled') + (current ? ' •' : '') + '</div><div class="file-meta">' + escapeHtml(meta) + '</div></div>' +
