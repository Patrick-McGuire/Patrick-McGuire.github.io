...
class BleWriter {
  ...
  async _writeChunks(data) {
    // Per spec, writeValueWithoutResponse() exists on every characteristic
    // regardless of whether the peripheral actually supports it -- the
    // browser is supposed to reject the *call* at runtime if not, not omit
    // the method. So `'writeValueWithoutResponse' in this._char` (which
    // this used to check) doesn't actually tell you anything; the real
    // answer is the characteristic's declared properties. This module's
    // write characteristic (FFF2) is documented read/write only (no
    // write-without-response), and a strict implementation enforcing that
    // is exactly what turns into "a network error occurred" here.
...
// ran, so navigator.bluetooth can still be undefined right here even though
// it shows up a moment later. Recheck for a couple seconds instead of
// locking in a possibly-stale "unsupported" verdict from a single read.
let serialListenerAttached = false;
function updateApiSupportUi() {
  if (state.transport) return; // don't fight setConnected() once a session is live
  ui.unsupported.style.display = (!navigator.serial && !navigator.bluetooth) ? 'block' : 'none';
  ui.connectBtn.disabled = !navigator.serial;
  ui.connectBluetoothBtn.disabled = !navigator.bluetooth;
  if (navigator.serial && !serialListenerAttached) {
...
updateApiSupportUi();
let apiSupportChecks = 0;
const apiSupportTimer = setInterval(() => {
  updateApiSupportUi();
  if (++apiSupportChecks >= 15) clearInterval(apiSupportTimer);
}, 200);
