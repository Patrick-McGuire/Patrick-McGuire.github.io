...
const MAX_RX_BUFFER_CHARS = 64 * 1024;
const MAX_CONSOLE_LINE_CHARS = 8192;
...
function appendRawBytes(bytes) {
  for (const b of bytes) state.rawBytes.push(b);
  const overflow = state.rawBytes.length - MAX_RAW_BYTES;
  if (overflow > 0) state.rawBytes.splice(0, overflow);
}

function truncateConsoleLine(text) {
  if (text.length <= MAX_CONSOLE_LINE_CHARS) return text;
  const hidden = text.length - MAX_CONSOLE_LINE_CHARS;
  return text.slice(0, MAX_CONSOLE_LINE_CHARS) + ` … [truncated ${hidden} chars]`;
}
...
function describePortOptions(opts) {
  const parity = opts.parity === 'none' ? 'N' : opts.parity.charAt(0).toUpperCase();
  const flow = opts.flowControl === 'hardware' ? ', hardware flow' : '';
  return `${opts.baudRate} baud, ${opts.dataBits}${parity}${opts.stopBits}${flow}`;
}
...
async function openPort(port) {
  ...
  try {
    ...
    try { await port.close(); } catch(closeErr) {}
    throw e;
  }
...
let lastReadStreamErrorWarnAt = 0;

function warnReadStreamError(error) {
  const now = Date.now();
  if (now - lastReadStreamErrorWarnAt < 2000) return;
  const msg = error && error.message ? error.message : String(error || 'Unknown read error');
  appendLog(`Serial read error: ${msg}. Retrying receive stream; check baud/parity/stop bits if this repeats.`, 'warn');
  lastReadStreamErrorWarnAt = now;
}

async function waitForReadable(port) {
  for (let i = 0; i < 20; i++) {
    if (!state.connected || state.port !== port) return false;
...
async function readLoop() {
  ...
  while (state.connected && state.port === port) {
    if (!port.readable && !(await waitForReadable(port))) break;
      ...
      while (state.connected && state.port === port) {
        ...
        if (value) {
          try {
            ...
            appendLog(`Receive processing error: ${e.message}`, 'error');
          }
        }
      }
    } catch(e) {
      if (state.connected && state.port === port) {
        // Wrong baud/parity commonly surfaces as a non-fatal stream error.
        // Keep the port open and let the browser recreate port.readable.
        warnReadStreamError(e);
        await delay(25);
      }
    } finally {
      if (reader) {
  ...
  if (state.connected && state.port === port && !state.reconfiguring) {
    appendLog('Receive stream stopped; closing port.', 'warn');
    await handleDisconnect(false);
  }
}

let rxBuffer = '';
let lastRxBufferDropWarnAt = 0;
...
function processIncomingBytes(bytes) {
  ...
  rxBuffer += text;
  if (rxBuffer.length > MAX_RX_BUFFER_CHARS) {
    const dropped = rxBuffer.length - MAX_RX_BUFFER_CHARS;
    rxBuffer = rxBuffer.slice(-MAX_RX_BUFFER_CHARS);
    const now = Date.now();
    if (now - lastRxBufferDropWarnAt > 1000) {
      appendLog(`Dropped ${dropped} chars from an unterminated receive line. Check baud rate or line endings.`, 'warn');
      lastRxBufferDropWarnAt = now;
    }
  }
  ...
  lines.forEach(line => {
    ...
    display = truncateConsoleLine(display);
    appendLog(display, 'rx');

    if (line.length <= MAX_CONSOLE_LINE_CHARS && !document.getElementById('plot-paused').checked) {
      parsePlotLine(line);
      schedulePlotDraw();
    }
  });
...
async function handleDisconnect(userInitiated = false) {
  ...
  if (wasConnected && !state.reconfiguring) appendLog(userInitiated ? 'Disconnected.' : 'Port disconnected (device removed?)', 'info');

  if (!state.reconfiguring && !userInitiated && state.shouldReconnect && state.settings.autoReconnect) {
    setStatus('waiting');
...
function queuePortReconfigure() {
  if (state.reconfiguring) {
    state.pendingReconfigure = true;
    return;
  }
  if (!state.connected || !state.port) return;
  ...
  reconfigureTimer = setTimeout(() => {
    ...
    reconfigurePortOptions();
  }, 100);
}

async function reconfigurePortOptions() {
  if (state.reconfiguring) {
    state.pendingReconfigure = true;
    return;
  }
  ...
  try {
    const before = currentPortOptions();
    appendLog(`Serial settings changed — reopening same port with ${describePortOptions(before)}…`, 'info');
    await handleDisconnect(true);

    const after = currentPortOptions();
    await openPort(port);
    appendLog(`Serial settings applied: ${describePortOptions(after)}`, 'info');
  } catch(e) {
    state.connected = false;
    state.connecting = false;
    state.shouldReconnect = false;
    ...
    document.getElementById('stat-port').textContent = 'No port';
    setStatus('');
    appendLog(`Failed to apply serial settings: ${e.message}`, 'error');
  } finally {
    state.reconfiguring = false;
    if (state.pendingReconfigure) {
      state.pendingReconfigure = false;
      if (state.connected) queuePortReconfigure();
    }
  }
}
...
serialOptionIds.forEach(id => {
  document.getElementById(id).addEventListener('change', queuePortReconfigure);
});
